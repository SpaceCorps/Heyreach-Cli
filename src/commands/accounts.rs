//! `accounts add|list|test|remove`. Manages named HeyReach accounts in keystore and config.
...
fn add(name: String, api_key: Option<String>, force: bool, no_verify: bool) -> Result<()> {
        ...
        && !force
    {
        return Err(Error::invalid(format!("An account named '{existing}' already exists.")).fix(format!(
            "Pick a different name, or replace its key: heyreach accounts add {existing} --api-key <key> --force"
        )));
    }
...
fn prompt_key(name: &str) -> Result<String> {
    if !std::io::stdin().is_terminal() {
        return Err(Error::invalid("No API key given and no terminal to prompt on.")
            .fix(format!("pbpaste | heyreach accounts add {name} --api-key-stdin")));
    }
    loop {
...
fn status_of(name: &str, store: Store, check: bool) -> String {
    ...
    match Client::new(&key, false).get("auth/CheckApiKey") {
        Ok(_) => "valid".into(),
...
fn test(name: &str) -> Result<()> {
    ...
    let client = account.client(false);
    client.get("auth/CheckApiKey")?;

    print(obj! {
        "name" => account.name,
        "identity" => account.config.identity,
        "keyStatus" => "valid",
    });
    Ok(())
}
...
fn remove(requested: &str, yes: bool) -> Result<()> {
    ...
    let Some((name, acct)) = config.find(requested) else {
        return Err(
            Error::new(ErrorCode::NoAccount, format!("No account named '{requested}'.")).fix("heyreach accounts list")
        );
    };
    ...
    if !yes {
        if !std::io::stdin().is_terminal() {
            ...
            .fix(format!("heyreach accounts remove {name} --yes")));
        }
    ...
    print(obj! {
        ...
        "note" => "The API key was deleted locally. Revoke it in your HeyReach settings if it should stop working everywhere.",
    });
    Ok(())
}
