...
//! refuses to start rather than silently writing a file - `HEYREACH_ALLOW_PLAINTEXT_STORE=1` is
...
const SERVICE: &str = "heyreach-cli";
...
/// The backend for this process. Cached: changing `HEYREACH_SECRET_STORE` mid-process has no
...
fn build() -> Result<Store> {
    if let Some(forced) = std::env::var("HEYREACH_SECRET_STORE").ok().filter(|s| !s.trim().is_empty()) {
        return match forced.to_lowercase().as_str() {
            ...
            _ => Err(Error::invalid(format!(
                "HEYREACH_SECRET_STORE='{forced}' is not a backend available on this platform."
            ))
            .fix("Unset HEYREACH_SECRET_STORE, or set it to one of: dpapi, keychain, libsecret, plaintext.")),
        };
    }
...
fn fallback(reason: &str) -> Result<Store> {
    if std::env::var("HEYREACH_ALLOW_PLAINTEXT_STORE").as_deref() == Ok("1") {
        return Ok(Store::Plaintext);
    }
    Err(Error::other("No secure credential store is available on this machine.")
        .detail(reason)
        .fix("Install a keystore, or set HEYREACH_ALLOW_PLAINTEXT_STORE=1 to store API keys in a 0600 file instead."))
}
...
impl Store {
    /// Backend name, as reported by `heyreach accounts list`.
    ...
    pub fn set(self, key: &str, value: &str) -> Result<()> {
        match self {
            ...
            Store::LibSecret => {
                // The secret goes in on stdin rather than argv, so it never appears in the
                // process list.
...
/// Warns on every process that reads the plaintext store rather than only when an account is
/// added, because the whole risk of this backend is that you forget you chose it.
fn warn_plaintext() {
    static WARNED: OnceLock<()> = OnceLock::new();
    WARNED.get_or_init(|| {
        eprintln!(
            "warning: API keys are stored unencrypted in {} (HEYREACH_ALLOW_PLAINTEXT_STORE=1).",
...
#[cfg(windows)]
mod dpapi {
    //! A single JSON blob encrypted with DPAPI under the current user, so it can only be
    ...
    pub fn unprotect(data: &[u8], path: &Path) -> Result<Vec<u8>> {
        ...
        if ok == 0 {
            return Err(Error::other("The stored API keys could not be decrypted.")
                ...
                .fix(format!("Delete {} and run: heyreach accounts add <name>", path.display())));
        }
        Ok(take(out))
    }
}
